...
@Tag(name = "PedidoController", description = "Controller responsável por gerenciar os pedidos")
@RequestMapping("/pedidos")
public interface PedidoController {

    @Operation(summary = "Listar pedidos")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Pedidos encontrados com sucesso", content = @Content(
    ...
    @GetMapping
    ResponseEntity<Page<PedidoResponse>> findAll(@RequestParam(required = false) UUID clienteId,
                                                 @ParameterObject Pageable pageable);

    @Operation(summary = "Buscar pedido por ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Pedido encontrado com sucesso", content = @Content(
                    mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = PedidoResponse.class)
            )),
            @ApiResponse(responseCode = "400", description = "Bad request", content = @Content(
                    mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = StandardError.class))),
    ...
    @GetMapping("/{id}")
    ResponseEntity<PedidoResponse> findById(@PathVariable UUID id);
    ...
    @PostMapping
    ResponseEntity<PedidoResponse> create(@RequestBody @Valid CreatePedidoRequest request);
    ...
    @PutMapping("/{id}")
    ResponseEntity<PedidoResponse> update(@PathVariable UUID id,
                                          @RequestBody @Valid UpdatePedidoRequest request);


    @Operation(summary = "Criar um novo pagamento para um pedido")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Pagamento adicionado com sucesso", content = @Content(
                    mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = PedidoResponse.class)
            )),
            @ApiResponse(responseCode = "400", description = "Bad request", content = @Content(
                    mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = StandardError.class))),
    ...
    ResponseEntity<Void> adicionarNovoPagamento(@PathVariable UUID id, @RequestBody CreateNovoPagamentoRequest request);
}
